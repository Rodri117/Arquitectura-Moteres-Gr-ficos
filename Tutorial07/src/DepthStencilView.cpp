...
DepthStencilView::init(Device device,
	...
	if (device.m_device == nullptr) {
		WARNING("ERROR: DepthStencilView::init : [CRATION OF RESOURCE : FALSE] [CHECK FOR Device device] \n");
		exit(1);
	}
	else if (depthStencil == nullptr) {
		WARNING("ERROR: DepthStencilView::init : [CREATION OF RESOURCE : FALSE] [CHECK FOR ID3D11Resorce* depthStencil] \n")
		exit(1);
	}
	else {
		//se crea el deviceview
